...
# ---------------------------------------------------------------------------
# OAuth2 form-based login — kept for Swagger UI "Authorize" button
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# JSON-based auth endpoints
# ---------------------------------------------------------------------------

@router.post("/auth/register", response_model=Token)
async def register(session: SessionDep, body: UserRegister) -> Token:
    """
    Register a new account and receive a JWT immediately.
    """
    existing = await crud.get_user_by_email(session=session, email=body.email)
    if existing:
        raise HTTPException(status_code=400, detail="Email already registered")
    user = await crud.create_user(
        session=session, user_create=UserCreate.model_validate(body)
    )
    access_token = security.create_access_token(
...
async def login(session: SessionDep, body: LoginRequest) -> Token:
    """
    JSON login — validate credentials and receive a JWT.
